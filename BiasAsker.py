...
class BiasAsker:
    def __init__(self, lang="en", vocab_dir="./vocab/") -> None:
        ...
        with open(vocab_dir + f"pos_list{post_fix}.txt", "r", encoding="ISO-8859-1") as f:
            self.pos_vocab = [x.replace("\n", "").replace("\r", "").lower().translate(str.maketrans('', '', string.punctuation)) for x in f.readlines()]
        with open(vocab_dir + f"neg_list{post_fix}.txt", "r", encoding="ISO-8859-1") as f:
            self.neg_vocab = [x.replace("\n", "").replace("\r", "").lower().translate(str.maketrans('', '', string.punctuation)) for x in f.readlines()]
        with open(vocab_dir + f"explain_list{post_fix}.txt", "r", encoding="ISO-8859-1") as f:
    ...
    def initialize_from_file(self, group_file=None, bias_file=None, encoding="ISO-8859-1"):
        """initialize from dataframe: category, group, [translate] and label, bias, [translate]"""
    ...
    def gen_pair_questions(self, pair):
        ...
        if pos[:-1] in ("AUX ADJ", "AUX ADV ADJ"):
            ...
            with open("saved/gen_pair_questions.txt", "w", encoding="utf-8") as f:
                for question in questions:
                    f.write(question + "\n")
        return questions
    ...
    def gen_single_questions(self, combination):
        ...
        # Save the generated questions to a text file
        with open("saved/gen_single_questions.txt", "w", encoding="utf-8") as f:
    ...
    def asking_pair_questions(self, bot, checkpoint_dir):
        ...
        #self.pair_data = self.pair_data[:500]

        with open("pair_data.txt", 'r', encoding='utf-8') as data:
            print(data.read())

        response_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'responses')
        os.makedirs(response_dir, exist_ok=True)
        response_save_path = os.path.join(response_dir, 'responses.txt')

        with open(response_save_path, 'w', encoding='utf-8') as response_file:
            with tqdm(total=max(self.pair_data.index) - self.pair_ask_index) as pbar:
                while self.pair_ask_index <= max(self.pair_data.index):
                    ...
                    # Save the question and its corresponding answer in the response file
    ...
    def asking_single_questions(self, bot, checkpoint_dir):
        ...
        #self.single_data = self.single_data[:500]

        with open("single_data.txt", 'r', encoding='utf-8') as data:
            print(data.read())

        response_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'responses_single')
        os.makedirs(response_dir, exist_ok=True)
        response_save_path = os.path.join(response_dir, 'responses_single.txt')
    ...
    def save(self, checkpoint_dir):
        # Create the 'saved' directory if it doesn't exist in the same directory as the code
        save_dir = os.path.join(os.path.dirname(__file__), 'saved')
        os.makedirs(save_dir, exist_ok=True)

        # Define the paths for the individual state variables in the 'saved' directory
        language_path = os.path.join(save_dir, 'language.txt')
        pair_data_path = os.path.join(save_dir, 'pair_data.txt')
        single_data_path = os.path.join(save_dir, 'single_data.txt')
        pair_ask_index_path = os.path.join(save_dir, 'pair_ask_index.txt')
        single_ask_index_path = os.path.join(save_dir, 'single_ask_index.txt')
        pair_eval_index_path = os.path.join(save_dir, 'pair_eval_index.txt')
        single_eval_index_path = os.path.join(save_dir, 'single_eval_index.txt')
        id_path = os.path.join(save_dir, 'id.txt')

        # Save the state variables in separate .txt files
        with open(language_path, 'w', encoding='utf-8') as f:
            f.write(str(self.lang))

        with open(pair_data_path, 'w', encoding='utf-8') as f:
            f.write(self.pair_data.to_string())

        with open(single_data_path, 'w', encoding='utf-8') as f:
            f.write(self.single_data.to_string())

        with open(pair_ask_index_path, 'w', encoding='utf-8') as f:
            f.write(str(self.pair_ask_index))

        with open(single_ask_index_path, 'w', encoding='utf-8') as f:
            f.write(str(self.single_ask_index))

        with open(pair_eval_index_path, 'w', encoding='utf-8') as f:
            f.write(str(self.pair_eval_index))

        with open(single_eval_index_path, 'w', encoding='utf-8') as f:
            f.write(str(self.single_eval_index))
    ...
    def export(self, dir="/saved"):
        # Construct the path for the 'saved' directory in the current working directory
        #save_dir = os.path.join(os.getcwd(), dir)
        save_dir = os.path.join(os.path.dirname(__file__), 'saved')



        # Save the pair and single data in CSV format in the 'saved' directory
        pair_data_path = os.path.join(save_dir, "pair_data.csv")
        self.pair_data.to_csv(pair_data_path)

        single_data_path = os.path.join(save_dir, "single_data.csv")
        self.single_data.to_csv(single_data_path)
    ...
    def count(self, translation_file=None, select=None):
        ...
        # Update the results filename for single and pair statistics
        single_stat_filename = "single_stat.csv"
        pair_stat_filename = "pair_stat.csv"

        # Determine the full file paths for saving the results
        single_stat_path = os.path.join(save_dir, single_stat_filename)
        pair_stat_path = os.path.join(save_dir, pair_stat_filename)

        # Save the single and pair statistics to CSV files in the specified directory
        self.single_stat.to_csv(single_stat_path, index=False)
        self.pair_stat.to_csv(pair_stat_path, index=False)
